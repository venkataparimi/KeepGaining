"""
Compute technical indicators for candle data - BATCH VERSION.
Processes multiple instruments and uses index-free bulk loading for speed.
"""
import asyncio
import asyncpg
import numpy as np
...
def compute_indicators_for_data(instrument_id, timeframe, timestamps, high, low, close, volume):
    """Compute all indicators for raw data arrays. Returns list of record tuples."""
    n = len(timestamps)
    if n < 200:
        return []
...
async def main(instrument_type: str = None, limit: int = 100):
    ...
    print("=" * 60)
    print("INDICATOR COMPUTATION (BATCH MODE)")
    print("=" * 60)
    ...
    # Step 1: Drop indexes for fast loading
    print("\nDropping indexes for fast loading...")
    ...
    # Step 2: Set replication mode to skip FK triggers
    ...
    for i, inst in enumerate(instruments):
        try:
            ...
            # Compute indicators
            records = compute_indicators_for_data(instrument_id, '1m', timestamps, high, low, close, volume)
            ...
            if (i + 1) % 5 == 0:
                ...
                print(f"Progress: {i + 1}/{len(instruments)} | Records: {total_records:,} | Rate: {rate:.2f}/s | {inst['trading_symbol'][:15]}")
    ...
    # Step 3: Reset session and recreate indexes
    await conn.execute("SET session_replication_role = 'origin'")
    
    print("\nRecreating indexes...")
    t1 = datetime.now()
    await conn.execute('CREATE INDEX idx_indicator_time ON indicator_data (timestamp)')
    await conn.execute('CREATE INDEX idx_ind_instrument_time ON indicator_data (instrument_id, timestamp)')
    t2 = datetime.now()
    print(f"Index rebuild: {(t2-t1).total_seconds():.1f}s")
